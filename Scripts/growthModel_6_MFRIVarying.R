...
#         MAP <- rnorm(1,mean=Combinations$RainfallRange[m],sd=Combinations$RainfallRange[m] * 0.22294 + 107.76283)}
  ...
  treeHeightsByMAPbyMFRI_df <- data.frame(MAP = numeric(0), Height = numeric(0),MFRI=numeric(0))

  timeLimit <- 5
  treeNumbers <- 1000
  FireReturnInterval <- 2
  #MAP_Series <- c(450,500,750)
  MAP_Series <- seq(350,725,25)
  FRI_series <- seq(2,5,1)


  runs <- 1000

for(FRI_iterator in FRI_series)
{
...
treeHeightsByMAP_df_long$MFRI <- as.numeric(FRI_iterator)

treeHeightsByMAPbyMFRI_df <- rbind(treeHeightsByMAPbyMFRI_df,treeHeightsByMAP_df_long)
...
pEscape <- ddply(.data = treeHeightsByMAPbyMFRI_df,
                 .(MAP,MFRI),
                 summarise,
...
escapePlot <- ggplot(data=pEscape,aes(x=MAP, y=pResult,color=MFRI))
escapePlot+
  myTheme+
  theme(text = element_text(size = 35),
        legend.background = element_rect(),
