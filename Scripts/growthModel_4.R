...
setwd("/Users/danielgodwin/Dropbox/Graduate School/Dissertation/FireEscapeModel/")
...
for(k in seq(treeNumbers)) #Generate a cohort of trees
    ...
    TESE <- savannaTree(ID = paste("TESE","-",FireReturnInterval,"-",MAP,"-","-",rnorm(1,100)), #Generates individuals in a cohort through a count of tree numbers, above.
                        ...
                        GrowthIntercept = 0,
                        GrowthA = 0, #Sets the A of the model
                        GrowthError = 0,
                        AnnualPrecipEst = 0,
                        InteractEst = 0,
                       )
    ...
    { 
      ...
      #Calculate height
      TESE$Height <- exp(predict(TESE_ll,list(Diameter=TESE$basalDiameter))[1])
      COMO$Height <- exp(predict(COMO_ll,list(Diameter=COMO$basalDiameter))[1])
      COAP$Height <- exp(predict(COAP_ll,list(Diameter=COAP$basalDiameter))[1])
      
      # Figure out how much each tree would grow if it doesn't get top-killed
      
      if(RandomGrowth == TRUE){
         
        COMO_Growth_Step <- exp(predict(COMO_naiveGrowth,list(Age=COMO$PersonalClock))[1])
        COAP_Growth_Step <- exp(predict(COAP_naiveGrowth,list(Age=COAP$PersonalClock))[1])
        TESE_Growth_Step <- exp(predict(TESE_naiveGrowth,list(Age=TESE$PersonalClock))[1])
